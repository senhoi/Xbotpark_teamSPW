...
float PID_Calc(PID_t *pid, float fbk)
{
	...
	pid->err = pid->ref - pid->fbk;

	switch (pid->mode)
	{
	case PID_Mode_Regular:
		if (pid->int_sp.enable == true && fabs(pid->err) > pid->int_sp.threshold)
		{
		...
		else
		{
			...
			pid->err_sum += deadband(pid->deadband, pid->err);
	...
	case PID_Mode_Increment:
		pid->inc = pid->Kp * (pid->err - pid->prev_err) + pid->Ki * pid->err + pid->Kd * (pid->err - 2 * pid->prev_err + pid->last_err);
		pid->inc = abslimit(pid->max_inc, pid->inc);
		pid->out = pid->prev_out + pid->inc;
		break;
